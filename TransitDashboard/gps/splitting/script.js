...
function checkLoaded() {
	toLoad--;
	if(toLoad == 0) {
		$('#title').html('Temporary Page for Trip Splitting');
...
function getTripData() {
	loadData('../data-splitting/trip_list.csv', function(data) {
	...
	loadData('../data-splitting/gps_data.csv', function(data) {
		tripData = data;
		checkLoaded();
	});
	loadData('../data-splitting/gps_index.csv', function(data) {
		tripIndex = data;
		checkLoaded();
	});
	loadData('../data-splitting/gps_cum.csv', function(data) {
		cumData = data;
		checkLoaded();
	}, false);
	$.getJSON('../data-splitting/map_styles.json',function(data) {
		mapStyles = data;
		checkLoaded();
	});
}
...
function drawMap(start, end) {
	...
	var newPath = [];
	for(i = start; i < end; i++) {
		newPath[i-start] = new google.maps.LatLng(tripData[i]['Latitude'], tripData[i]['Longitude']);
	}
...
function addMarker(i) {
	var tmpPos = new google.maps.LatLng(tripData[i]['Latitude'], tripData[i]['Longitude']);
